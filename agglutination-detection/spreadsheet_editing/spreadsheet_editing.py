...
import argparse
import numpy as np
from spreadsheet_editing.spreadsheet_constants import SCOPES, SPREADSHEET_ID, RANGE_NAME
import sys
sys.path.insert(0,'..')
from constants import ANNOTATOR_COLS
import time
...
def update_spreadsheet_labels(label_update_dict, token_path=f".{os.sep}token.json", creds_path=f".{os.sep}credentials.json"):
    """
        Updates a dict of labels to our google spreadsheet.
    """
    service = get_spreadsheet_service(token_path, creds_path)

    existing_urls = read_spreadsheet(service)[0]

    for url in tqdm(label_update_dict):
        well_ind = existing_urls.index(url)
        label = label_update_dict[url]

        # Update the label for each annotator
        for annotator_col in ANNOTATOR_COLS:
            try:
                value_input_option = 'USER_ENTERED'
                range_ = f"{RANGE_NAME}!{annotator_col}{well_ind + 1}"
                value_range_body = {
                    "majorDimension": "ROWS",
                    "values": [[label]]
                }
                ...
                print("Got HTTP429 error! Going to wait, then try again.")
                time.sleep(70)
                value_input_option = 'USER_ENTERED'
                range_ = f"{RANGE_NAME}!{annotator_col}{well_ind + 1}"
                value_range_body = {
                ...
                request = service.spreadsheets().values().update(spreadsheetId=SPREADSHEET_ID, range=range_, valueInputOption=value_input_option, body=value_range_body)
                response = request.execute()
        print(f"Finished labelling {url}")
